...
function _switchScreen(name) {
  ...
  if (name === "title") {
    ...
    const requestedMood = screens.officialSelect?.dataset.restoreExMood;
    const keepExMood = requestedMood === "true";
    if (!keepExMood) {
    ...
    setGlobalGlitchTarget(0);
    playStageBgm(keepExMood ? "vertex" : "warm");
    setStageTheme(keepExMood ? "space" : "warm");
    setShowDlcPillars(false);
