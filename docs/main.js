...
export function updateAllVolumes() {
  ...
  // ★追加：スライダーで0から上げた時に再生が始まるようにする
  if (audioSettings.bgmVolume > 0 && currentBgmKey) {
    const targetAudio = bgms[currentBgmKey] || bgms.warm;
    if (targetAudio && targetAudio.paused) {
      playStageBgm(currentBgmKey);
    }
  } else if (audioSettings.bgmVolume <= 0) {
    // ★追加：音量を0にした瞬間に全てのBGMを物理的にストップ（オーディオフォーカスを解放）
    Object.values(bgms).forEach(audio => {
      if (audio) {
        audio.pause();
        audio.currentTime = 0;
...
export function playChin() {
  // ★修正：音量が0なら何もしない（再生処理を呼ばない）
  if (audioSettings.sfxVolume <= 0) return;

  if (seChin) {
    seChin.currentTime = 0;
    seChin.volume = audioSettings.sfxVolume; // 0.6固定から変更
    seChin.play().catch(() => { });
  }
}
...
export function playStageBgm(themeKey) {
  ...
  // ★追加：音量が0なら再生処理そのものを完全にスキップ
  if (audioSettings.bgmVolume <= 0) {
    currentBgmKey = themeKey; // キーだけ保持
    return;
  }
