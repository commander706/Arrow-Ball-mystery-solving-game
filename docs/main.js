...
export function playSound(name, volumeScale = 1.0) {
  ...
  if (volume <= 0 || !soundBuffers[name]) return;
...
const chkShowTrail = document.getElementById("chkShowTrail"); // ★追加
...
export function updateAllVolumes() {
  ...
  // 音量を0にした場合はBGMを物理的に止める（オーディオフォーカス解放のため）
...
export function playStageBgm(themeKey) {
  ...
  if (currentBgmGain) {
    ...
    oldGain.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + 1.0);
    setTimeout(() => {
  ...
  gainNode.gain.linearRampToValueAtTime(audioSettings.bgmVolume, audioCtx.currentTime + 1.0);
...
function initDlcThreeJs() {
  ...
  // ★修正: ウィンドウ全体ではなくコンテナのサイズを取得
  const width = container.clientWidth;
  const height = container.clientHeight;

  // 1. シーン & フォグ設定
  dlcScene = new THREE.Scene();
  const fogColor = new THREE.Color(0x12252e);
  ...
  // 3. レンダラー設定
  dlcRenderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
  dlcRenderer.setSize(width, height);
  dlcRenderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  ...
  // 4. マテリアル設定: 立体感（面ごとの色変え）を追加
  const pillarMaterial = new THREE.ShaderMaterial({
    uniforms: {
      colorBottom: { value: new THREE.Color(0x0d1a21) },
      // ★修正: 柱の上部をもう少し水色っぽく明るく（でも暗さは残す）
      colorTop: { value: new THREE.Color(0x38606b) },
      fogColor: { value: fogColor },
      fogDensity: { value: 0.02 }
    },
    // ★修正: 法線(normal)を受け取るように変更
    vertexShader: `
      varying vec2 vUv;
      varying float vDist;
  ...
  for (let i = 0; i < pillarCount; i++) {
    const mesh = new THREE.Mesh(geometry, pillarMaterial.clone());
    ...
    // ★修正: 中心からの距離に応じて高さを制限する
    // 遠くや端に行くほど高くならないように係数をかける
    const distFromCenter = Math.sqrt(x * x + z * z);
    // 距離が遠いほど高さを抑える (最大で元の0.3倍くらいまで小さくする)
...
// ★追加: 外部からDLC柱の表示/非表示を制御する関数
export function setShowDlcPillars(enable) {
...
export function setStageTheme(themeName) {
  ...
  // const dlcLayer = ... (ここでは制御しない)

  // 1. 全テーマのクラスとアクティブ状態をリセット
  bgEl.className = "bg";

  // ★修正: dlcLayerのリセットはここでは行わない（独立制御のため）
  ...
  // stopDlcThreeJs(); // ★削除: ここで止めると柱が消えてしまう
  ...
  switch (themeName) {
    ...
    case "dlc_world":
      ...
      // ★修正: ここでの Three.js 起動処理は削除 (setShowDlcPillars に任せる)
      break;
...
if (btnGlobalResetMain) {
  btnGlobalResetMain.addEventListener("click", () => {
    if (confirm("本当にメインステージの進行データを削除しますか？\n（クリア状況・EX/DLC開放状態がリセットされます）")) {
      ...
      // ★追加: もしエディター側の読み込み関数が公開されていれば実行して、
      // メモリ上のレベルリスト(DLC含む)をリセット・再描画する
...
// 右クリックメニュー（コンテキストメニュー）を無効化
document.addEventListener('contextmenu', (e) => {
  e.preventDefault();
}, { passive: false });

// ドラッグによるテキスト選択を無効化（入力フォームは許可）
document.addEventListener('selectstart', (e) => {
  // 入力可能な要素(INPUT, TEXTAREAなど)なら選択を許可
