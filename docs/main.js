...
function fitCanvasToImage(img) {
  // Title logo should fit BOTH width and height (small desktop window / split view).
  const dpr = Math.max(1, window.devicePixelRatio || 1);

  // Available viewport size (use innerHeight; CSS uses svh for layout stability)
  const vw = Math.max(320, window.innerWidth || 0);
  const vh = Math.max(320, window.innerHeight || 0);

  // 1) Start from width-based size
  ...
  // 2) Cap by height (leave room for menu/buttons)
