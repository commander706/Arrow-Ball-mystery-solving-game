...
const GUARDIAN_PROGRESS_KEY = '3d_arrow_ball_guardian_progress';
const SANCTUARY_CLEAR_SHOWN_KEY = '3d_arrow_ball_sanctuary_clear_shown';
const SANCTUARY_STAGE_COUNT = 31;
const GUARDIAN_TIME_LIMIT_MS = 105000;
const GUARDIAN_PHASE_COUNT = 5;
const GUARDIAN_DEFINITIONS = Object.freeze({
  1: { roman: "I", name: "天輪の番人", afterStage: 16, unlockStage: 31 }
});
...
const SANCTUM_BOSS_STAGES = new Set([16, 30]);
function isBossLevel(level) {
  return level?._isDlc
    ? SANCTUM_BOSS_STAGES.has(Number(level._dlcNum))
    : !level?._isEx && MINI_BOSS_STAGES.has(level?._officialIndex);
}
...
let sanctuaryClearSequenceTimer = 0;
let immediateAllClearRequested = false;
let officialZoneMood = "main";
let exZonePulseTimer = 0;
let officialListDirty = true;
let lastOfficialPlayedIndex = -1;
let pendingOfficialReturnIndex = -1;
let guardianBattle = null;
let guardianTimerId = 0;
let guardianDeadlineTimerId = 0;
let guardianBombSource = null;
const guardianSequenceTimers = new Set();
...
const guardianCoreVisual = document.getElementById("guardianCoreVisual");
const guardianCore = document.getElementById("guardianCore");
const guardianWarningOverlay = document.getElementById("guardianWarningOverlay");
const guardianWarningCode = document.getElementById("guardianWarningCode");
const guardianPhaseBadge = document.getElementById("guardianPhaseBadge");
const guardianFailureOverlay = document.getElementById("guardianFailureOverlay");

const allClearOverlay = document.getElementById("allClearOverlay");
const congratsText = document.getElementById("congratsText");
const sanctuaryCongratsText = document.getElementById("sanctuaryCongratsText");
  ...
  if (btnResetProgress) {
    btnResetProgress.addEventListener("click", () => {
      if (confirm("本当にメインステージの進行データを削除しますか？\n（クリア状況・EX/聖域の開放状態がリセットされます）")) {
        ...
        localStorage.removeItem(GUARDIAN_PROGRESS_KEY);
        localStorage.removeItem(SANCTUARY_CLEAR_SHOWN_KEY);
        loadOfficialLevels();
...
async function loadGuardianPhaseLevels(loader, loadedCount, estimatedTotal) {
  guardianLevels = { 1: [] };

  for (const guardianId of [1]) {
    for (let phase = 1; phase <= GUARDIAN_PHASE_COUNT; phase++) {
      try {
        const path = `./dlc_levels/level_guardian_${guardianId}_phase_${phase}.3aab`;
        ...
        if (!data) continue;
        data._isDlc = true;
        data._isGuardian = true;
        data._guardianId = guardianId;
        data._guardianPhase = phase;
        data._officialId = `guardian-${guardianId}-phase-${phase}`;
        guardianLevels[guardianId][phase - 1] = data;
        loadedCount++;
        loader?.update(
          (loadedCount / estimatedTotal) * 100,
          `GUARDIAN ${guardianId} / PHASE ${phase} を読み込み中...`
        );
      } catch (e) {
        console.warn(`Guardian phase load failed: ${guardianId}-${phase}`, e);
      }
    }
  }
...
window.loadOfficialLevels = async function () {
  ...
  if (isDlcUnlocked) {
    ...
    loadedCount = await loadGuardianPhaseLevels(loader, loadedCount, ESTIMATED_TOTAL);
  } else {
    guardianLevels = { 1: [] };
  }
...
function getGuardianProgress() {
  try {
    return JSON.parse(localStorage.getItem(GUARDIAN_PROGRESS_KEY) || "{}") || {};
  } catch (e) {
    return {};
  }
}

function isGuardianCleared(guardianId, progress = getGuardianProgress()) {
  return Boolean(progress[String(guardianId)]?.cleared);
}

function saveGuardianCleared(guardianId) {
  const progress = getGuardianProgress();
  progress[String(guardianId)] = { cleared: true, clearedAt: Date.now() };
  localStorage.setItem(GUARDIAN_PROGRESS_KEY, JSON.stringify(progress));
  officialListDirty = true;
}
...
function areAllSanctuaryStagesCleared(clearedStages = getClearedStages()) {
  const sanctuaryLevels = officialLevels.filter(level => level._isDlc);
  return sanctuaryLevels.length >= SANCTUARY_STAGE_COUNT && sanctuaryLevels
    .slice(0, SANCTUARY_STAGE_COUNT)
    .every(level => Boolean(clearedStages[level._officialIndex]));
}
...
function getVisibleSanctumLimit(guardianProgress = getGuardianProgress()) {
  if (isGuardianCleared(1, guardianProgress)) return SANCTUARY_STAGE_COUNT;
  return 16;
}
...
function areGuardianPhasesReady(guardianId) {
  return Array.from(
    { length: GUARDIAN_PHASE_COUNT },
    (_, index) => Boolean(guardianLevels[guardianId]?.[index])
  ).every(Boolean);
}

async function ensureGuardianPhasesLoaded(guardianId) {
  if (areGuardianPhasesReady(guardianId)) return true;
  if (!guardianLevels[guardianId]) guardianLevels[guardianId] = [];

  for (let phase = 1; phase <= GUARDIAN_PHASE_COUNT; phase++) {
    if (guardianLevels[guardianId][phase - 1]) continue;
    try {
      const path = `./dlc_levels/level_guardian_${guardianId}_phase_${phase}.3aab`;
      ...
      console.warn(`Guardian phase reload failed: ${guardianId}-${phase}`, error);
    }
  }

  return areGuardianPhasesReady(guardianId);
}

function appendGuardianCard(guardianId, guardianProgress) {
  const definition = GUARDIAN_DEFINITIONS[guardianId];
  const isReady = areGuardianPhasesReady(guardianId);
  const isCleared = isGuardianCleared(guardianId, guardianProgress);
  const item = document.createElement("div");
  item.className = "guardian-stage-card";
  if (isCleared) item.classList.add("cleared");
  if (!isReady) item.classList.add("data-missing");
  item.innerHTML = `
    <span class="guardian-card-scan" aria-hidden="true"></span>
    <span class="guardian-card-index">BOSS</span>
    <small>CRITICAL ENCOUNTER</small>
    <h4>GUARDIAN ${definition.roman}</h4>
    <p>${definition.name}</p>
    <div class="guardian-card-status">${isCleared ? "DEFEATED" : isReady ? "5 PHASES READY" : "PHASE DATA MISSING"}</div>
    ${isCleared ? `<span class="guardian-defeated-mark">★<i>DEFEATED</i></span>` : ""}
  `;
  item.tabIndex = 0;
  item.addEventListener("click", () => {
    playChin();
    startGuardianBattleWithLoading(guardianId);
  });
...
function renderOfficialList() {
  ...
  const sanctuaryProgress = document.getElementById("officialSanctuaryProgress");
  ...
  const firstDlcLevel = officialLevels.find(level => level._isDlc);
  const dlcStartIndex = firstDlcLevel?._officialIndex ?? (MAIN_STAGE_COUNT + 10);
  const sanctuaryClearedCount = officialLevels.filter(
    level => level._isDlc && Boolean(clearedStages[level._officialIndex])
  ).length;
  if (sanctuaryProgress) {
    const count = sanctuaryProgress.querySelector("strong");
    if (count) count.textContent = String(sanctuaryClearedCount);
    sanctuaryProgress.hidden = !hasDlcUnlocked;
  }
  const guardianProgress = getGuardianProgress();
  const clearedDlcIndices = clearedIndices.filter(index => index >= dlcStartIndex);
  const maxClearedDlc = clearedDlcIndices.length > 0 ? Math.max(...clearedDlcIndices) : dlcStartIndex - 1;
  const visibleSanctumLimit = getVisibleSanctumLimit(guardianProgress);
  ...
  let sanctuaryBoundaryAdded = false;
  ...
  officialLevels.forEach((level, idx) => {
    ...
    if (level._isDlc && level._dlcNum > visibleSanctumLimit) return;
    ...
    if (level._isDlc && !sanctuaryBoundaryAdded) {
      sanctuaryBoundaryAdded = true;
      const boundary = document.createElement("div");
      boundary.className = "sanctuary-zone-boundary";
      boundary.innerHTML = `
        <div class="sanctuary-current-line"><i></i><i></i><i></i></div>
        <div class="sanctuary-boundary-copy">
          <span>RESTRICTED DOMAIN</span>
          <strong>ここから先は 聖域</strong>
    ...
    if (level._isDlc) isUnlocked = idx <= maxClearedDlc + 1;

    if (!isUnlocked) item.classList.add("locked");
    if (isUnlocked && !isCleared && !level._isEx) item.classList.add("current-stage");
    ...
    if (level._isDlc && isCleared) {
      const guardianId = level._dlcNum === GUARDIAN_DEFINITIONS[1].afterStage ? 1 : 0;
      if (guardianId) appendGuardianCard(guardianId, guardianProgress);
    }
  });
  ...
  if (hasDlcUnlocked && visibleSanctumLimit < SANCTUARY_STAGE_COUNT) {
    const guardianId = 1;
    const definition = GUARDIAN_DEFINITIONS[guardianId];
    const bossProgressIndex = dlcStartIndex + definition.afterStage - 1;
    const guardianAvailable = Boolean(clearedStages[bossProgressIndex]);
    const sectionGate = document.createElement("div");
    sectionGate.className = "main-section-gate sanctuary-section-gate";
    sectionGate.innerHTML = `
      <span>SANCTUARY ROUTE SEALED</span>
      <strong>${guardianAvailable ? `GUARDIAN ${definition.roman}を撃破せよ` : `聖域 ${definition.afterStage}のボスを突破せよ`}</strong>
      <small>突破後、聖域 ${definition.unlockStage}まで解放</small>
    `;
    officialListContainer.appendChild(sectionGate);
  }
  ...
  if (hasDlcUnlocked) {
    queueSanctuaryClearSequence();
  }
}
function updateExZoneMood() {
  const screen = document.getElementById("officialSelectScreen");
  const exBoundary = officialListContainer?.querySelector(".ex-zone-boundary");
  const sanctuaryBoundary = officialListContainer?.querySelector(".sanctuary-zone-boundary");
  if (!screen?.classList.contains("screen--active")) return;

  const listRect = officialListContainer.getBoundingClientRect();
  const hasReached = boundary => Boolean(
    boundary &&
    (!boundary.classList.contains("preunlock") || screen.classList.contains("ex-awakening")) &&
    boundary.getBoundingClientRect().top < listRect.bottom - 70
  );
  let nextMood = "main";
  if (hasReached(exBoundary)) nextMood = "ex";
  if (hasReached(sanctuaryBoundary)) nextMood = "sanctuary";

  const moodWasRendered = nextMood === "sanctuary"
    ? screen.classList.contains("near-sanctuary-zone")
    : nextMood === "ex"
      ? screen.classList.contains("near-ex-zone")
      : !screen.classList.contains("near-ex-zone") && !screen.classList.contains("near-sanctuary-zone");
  screen.classList.toggle("near-ex-zone", nextMood === "ex");
  screen.classList.toggle("near-sanctuary-zone", nextMood === "sanctuary");
  if ((nextMood === officialZoneMood && moodWasRendered) || allClearSequenceRunning) return;

  officialZoneMood = nextMood;
  setGlobalGlitchTarget(0);
  window.clearTimeout(exZonePulseTimer);
  screen.classList.remove("ex-glitch-pulse", "sanctuary-zone-pulse");
  if (nextMood === "ex") {
  ...
  } else if (nextMood === "sanctuary") {
    void screen.offsetWidth;
    screen.classList.add("sanctuary-zone-pulse");
    exZonePulseTimer = window.setTimeout(() => {
      screen.classList.remove("sanctuary-zone-pulse");
    }, 900);
  }
  setStageTheme(nextMood === "sanctuary" ? "dlc_world" : nextMood === "ex" ? "space" : "warm");
  playStageBgm(nextMood === "sanctuary" ? "sublime" : nextMood === "ex" ? "vertex" : "warm");
  setShowDlcPillars(nextMood === "sanctuary");
}
...
function queueSanctuaryClearSequence(delay = 260) {
  window.clearTimeout(sanctuaryClearSequenceTimer);
  const clearedStages = getClearedStages();
  if (
    !isDlcUnlocked() ||
    !areAllSanctuaryStagesCleared(clearedStages) ||
    localStorage.getItem(SANCTUARY_CLEAR_SHOWN_KEY) === "true"
  ) return;

  sanctuaryClearSequenceTimer = window.setTimeout(() => {
    sanctuaryClearSequenceTimer = 0;
    triggerSanctuaryClearSequence();
  }, delay);
}
...
function restoreOfficialSelectionPosition() {
  ...
  delete screen.dataset.restoreZone;
  requestAnimationFrame(() => {
    updateExZoneMood();
    queueAllClearSequence();
    queueSanctuaryClearSequence();
...
function leaveCurrentPlayScreen() {
  cancelSanctumReveal(playGrid);
  if (isOfficialPlay) {
    const returnIndex = currentLevel?._officialIndex ?? lastOfficialPlayedIndex;
    pendingOfficialReturnIndex = returnIndex;
    const returnZone = currentLevel?._isDlc ? "sanctuary" : currentLevel?._isEx ? "ex" : "main";
    const screen = document.getElementById("officialSelectScreen");
    if (screen) screen.dataset.restoreZone = returnZone;
    stopPlayMode(returnZone !== "main");
...
async function triggerSanctuaryClearSequence() {
  if (allClearSequenceRunning || allClearOverlay.classList.contains("active")) return;

  const screen = document.getElementById("officialSelectScreen");
  if (!screen?.classList.contains("screen--active") || !sanctuaryCongratsText) return;
  if (!areAllSanctuaryStagesCleared()) return;

  allClearSequenceRunning = true;
  localStorage.setItem(SANCTUARY_CLEAR_SHOWN_KEY, "true");
  screen.classList.add("finale-locked", "near-sanctuary-zone");
  screen.classList.remove("near-ex-zone", "ex-glitch-pulse");
  officialZoneMood = "sanctuary";

  const sanctuaryCards = Array.from(officialListContainer.querySelectorAll(".level-item.dlc"));
  const finalSanctuaryCard = sanctuaryCards.at(-1);
  if (finalSanctuaryCard) {
    officialListContainer.style.scrollBehavior = "auto";
    officialListContainer.scrollTop = getOfficialCardScrollTarget(finalSanctuaryCard, 0.46);
  }

  setStageTheme("dlc_world");
  setShowDlcPillars(true);
  fadeOutStageBgm(0.7);
  playSe("allClear");
  allClearOverlay.className = "all-clear-overlay active phase-congrats sanctuary-complete";
  sanctuaryCongratsText.classList.remove("finished");

  await waitForFinale(3300);
  if (!allClearSequenceRunning) return;
  sanctuaryCongratsText.classList.add("finished");
  await waitForFinale(650);
  if (!allClearSequenceRunning) return;
  ...
  allClearOverlay.className = "all-clear-overlay hidden";
  sanctuaryCongratsText.classList.remove("finished");
  screen.classList.remove("finale-locked");
  officialListContainer.style.scrollBehavior = "";
  allClearSequenceRunning = false;
  playStageBgm("sublime");
  updateExZoneMood();
}
...
function queueGuardianSequence(callback, delay) {
  const timerId = window.setTimeout(() => {
    guardianSequenceTimers.delete(timerId);
    callback();
  }, delay);
  guardianSequenceTimers.add(timerId);
  return timerId;
}

function stopGuardianClock() {
  if (guardianTimerId) window.clearInterval(guardianTimerId);
  if (guardianDeadlineTimerId) window.clearTimeout(guardianDeadlineTimerId);
  guardianTimerId = 0;
  guardianDeadlineTimerId = 0;
}

function stopGuardianBomb() {
  if (!guardianBombSource) return;
  try { guardianBombSource.stop(); } catch (e) { }
  guardianBombSource = null;
}

function clearGuardianSequenceTimers() {
  guardianSequenceTimers.forEach(timerId => window.clearTimeout(timerId));
  guardianSequenceTimers.clear();
}

function resetGuardianPresentation() {
  stopGuardianClock();
  clearGuardianSequenceTimers();
  stopGuardianBomb();
  ...
  screen?.classList.remove(
    ...
    "guardian-intro",
    "guardian-stage-live",
    "guardian-final-phase",
    "guardian-critical",
    "guardian-phase-shifting",
    "guardian-failing",
    "guardian-victory-sequence",
    "guardian-core-destroyed"
  );
  guardianCoreVisual?.classList.add("hidden");
  guardianCoreVisual?.classList.remove("assembling", "live", "shattered");
  guardianWarningOverlay?.classList.add("hidden");
  guardianWarningOverlay?.classList.remove("active", "victory");
  guardianPhaseBadge?.classList.add("hidden");
  guardianFailureOverlay?.classList.add("hidden");
  guardianFailureOverlay?.classList.remove("active", "black");
  screen?.querySelector(".guardian-victory-particle-layer")?.remove();
  if (guardianCore) guardianCore.innerHTML = "";
  guardianBattle = null;
}

function buildGuardianCore(guardianId) {
  if (!guardianCore) return;
  guardianCore.innerHTML = "";
  for (let i = 0; i < 14; i++) {
    const shard = document.createElement("i");
    const angle = (360 / 14) * i;
    shard.style.setProperty("--guardian-angle", `${angle}deg`);
    shard.style.setProperty("--guardian-counter-angle", `${-angle}deg`);
    shard.style.setProperty("--guardian-distance", `${125 + (i % 4) * 24}px`);
    shard.style.setProperty("--guardian-delay", `${i * 46}ms`);
    shard.style.setProperty("--guardian-depth", `${(i % 5) * 7}px`);
    guardianCore.appendChild(shard);
  }
  guardianCore.dataset.guardian = String(guardianId);
}

function formatGuardianTime(remainingMs) {
...
function updateGuardianClock() {
  if (!guardianBattle?.active || !guardianBattle.startedAt) return;
  const remaining = GUARDIAN_TIME_LIMIT_MS - (Date.now() - guardianBattle.startedAt);
  playIntroTitle.textContent = formatGuardianTime(remaining);
  document.getElementById("playScreen")?.classList.toggle("guardian-critical", remaining <= 30000);
  if (remaining <= 0) triggerGuardianFailure();
}

function startGuardianClock() {
  stopGuardianClock();
  guardianBattle.startedAt = Date.now();
  updateGuardianClock();
  guardianTimerId = window.setInterval(updateGuardianClock, 100);
  guardianDeadlineTimerId = window.setTimeout(triggerGuardianFailure, GUARDIAN_TIME_LIMIT_MS);
}

function stopGuardianBallAndEffects() {
  ...
  gsap.killTweensOf(playGrid);
  isBallMoving = false;
  isBallResetting = false;
  movementHistory = [];
  attemptStartedAt = 0;
  clearTransientEffects(playGrid);
  clearWarpConnections(playGrid);
}

function showGuardianPhase(phaseIndex, animateTransition = true) {
  if (!guardianBattle?.active) return;
  const phaseLevel = guardianLevels[guardianBattle.id]?.[phaseIndex];
  if (!phaseLevel) {
    triggerGuardianFailure();
    return;
  }

  const applyPhase = () => {
    if (!guardianBattle?.active) return;
    guardianBattle.phaseIndex = phaseIndex;
    guardianBattle.transitioning = false;
    currentLevel = JSON.parse(JSON.stringify(phaseLevel));
    originalLevelData = JSON.parse(JSON.stringify(currentLevel.data));
    resetGameState();
    renderGrid(playGrid);

    const screen = document.getElementById("playScreen");
    screen?.classList.remove("guardian-phase-shifting");
    screen?.classList.add("guardian-stage-live");
    screen?.classList.toggle("guardian-final-phase", phaseIndex === GUARDIAN_PHASE_COUNT - 1);
    guardianCoreVisual?.classList.remove("assembling");
    guardianCoreVisual?.classList.add("live");
    guardianPhaseBadge?.classList.remove("hidden");
    const phaseValue = guardianPhaseBadge?.querySelector("strong");
    if (phaseValue) phaseValue.textContent = `${phaseIndex + 1} / ${GUARDIAN_PHASE_COUNT}`;
    requestAnimationFrame(fitVisibleGrids);
    revealSanctumTiles(playGrid);
  };

  stopGuardianBallAndEffects();
  if (animateTransition) {
    guardianBattle.transitioning = true;
    document.getElementById("playScreen")?.classList.add("guardian-phase-shifting");
    queueGuardianSequence(applyPhase, 620);
  } else {
    applyPhase();
  }
}

function completeGuardianPhase(gx, gy, container) {
  if (!guardianBattle?.active || guardianBattle.transitioning) return;

  if (guardianBattle.phaseIndex === GUARDIAN_PHASE_COUNT - 1) {
    completeGuardianBattle(gx, gy, container);
    return;
  }
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const tile = getTileVisual(container, y * size + x);
      const delay = (Math.abs(x - gx) + Math.abs(y - gy)) * 32;
      if (tile) queueGuardianSequence(() => tile.classList.add("rainbow-effect"), delay);
    }
  }
  if (ballEl) gsap.to(ballEl, { scale: 0, opacity: 0, duration: 0.38 });

  queueGuardianSequence(() => {
    if (!guardianBattle?.active) return;
    showGuardianPhase(guardianBattle.phaseIndex + 1);
  }, 1050);
}

function spawnGuardianVictoryParticles(container) {
  const screen = document.getElementById("playScreen");
  if (!screen || !guardianCore) return;

  screen.querySelector(".guardian-victory-particle-layer")?.remove();
  const screenRect = screen.getBoundingClientRect();
  const sourceRect = container?.getBoundingClientRect();
  const targetRect = guardianCore.getBoundingClientRect();
  if (!sourceRect || !targetRect.width) return;

  const layer = document.createElement("div");
  layer.className = "guardian-victory-particle-layer";
  screen.appendChild(layer);

  const targetX = targetRect.left + targetRect.width / 2 - screenRect.left;
  const targetY = targetRect.top + targetRect.height / 2 - screenRect.top;
  for (let i = 0; i < 42; i++) {
    const particle = document.createElement("i");
    const startX = sourceRect.left - screenRect.left + sourceRect.width * (0.08 + Math.random() * 0.84);
    const startY = sourceRect.top - screenRect.top + sourceRect.height * (0.08 + Math.random() * 0.84);
    const curveX = (Math.random() - 0.5) * 150;
    const curveY = -40 - Math.random() * 100;
    particle.style.left = `${startX}px`;
    particle.style.top = `${startY}px`;
    particle.style.setProperty("--guardian-particle-size", `${3 + Math.random() * 6}px`);
    layer.appendChild(particle);
    particle.animate([
      { transform: "translate(-50%,-50%) scale(.2)", opacity: 0 },
      { transform: `translate(calc(-50% + ${curveX}px), calc(-50% + ${curveY}px)) scale(1.25)`, opacity: 1, offset: .28 },
      { transform: `translate(calc(-50% + ${targetX - startX}px), calc(-50% + ${targetY - startY}px)) scale(.05)`, opacity: 0 }
    ], {
...
function showGuardianClearScreen(guardianId) {
  const definition = GUARDIAN_DEFINITIONS[guardianId];
  clearOverlay.classList.remove("hidden");
  clearOverlay.classList.add("active");
  clearTextContainer.innerHTML = "";
  "CLEAR".split("").forEach((char, i) => {
  ...
  const actionsContainer = clearOverlay.querySelector(".clear-actions");
  if (!actionsContainer) return;
  actionsContainer.innerHTML = "";
  actionsContainer.classList.remove("final-main-actions");
  ...
  btnNext.onclick = () => {
    playChin();
    const nextIndex = officialLevels.findIndex(
      level => level._isDlc && level._dlcNum === definition.afterStage + 1
    );
    if (nextIndex >= 0) {
      startOfficialPlay(nextIndex);
      return;
    }
    leaveCurrentPlayScreen();
  };
  ...
  btnBack.onclick = () => {
    playChin();
    const returnLevel = officialLevels.find(
      level => level._isDlc && level._dlcNum === definition.afterStage
    );
    pendingOfficialReturnIndex = returnLevel?._officialIndex ?? -1;
    const selectScreen = document.getElementById("officialSelectScreen");
    if (selectScreen) selectScreen.dataset.restoreZone = "sanctuary";
  ...
  actionsContainer.append(btnNext, btnBack);
}

function completeGuardianBattle(gx, gy, container) {
  if (!guardianBattle?.active) return;
  const guardianId = guardianBattle.id;
  const definition = GUARDIAN_DEFINITIONS[guardianId];
  guardianBattle.active = false;
  guardianBattle.transitioning = true;
  stopGuardianClock();
  playSe("goal");
  clearPlayCoach();
  saveGuardianCleared(guardianId);

  const screen = document.getElementById("playScreen");
  screen?.classList.remove("guardian-critical");
  screen?.classList.add("guardian-victory-sequence");
  gsap.to(playIntro, { opacity: 0, duration: 0.45 });
  if (ballEl) gsap.to(ballEl, { opacity: 0, scale: 0.2, duration: 0.5, ease: "power2.in" });
  guardianPhaseBadge?.classList.add("hidden");

  queueGuardianSequence(() => {
    stopGuardianBallAndEffects();
    spawnGuardianVictoryParticles(container);
  }, 580);

  queueGuardianSequence(() => {
    guardianBombSource = playSound("seBomb", 0.5);
    fadeOutStageBgm(0.9);
    screen?.classList.add("guardian-core-destroyed");
    guardianCoreVisual?.classList.add("shattered");
    if (guardianWarningCode) guardianWarningCode.textContent = `GUARDIAN ${definition.roman} NEUTRALIZED`;
    const strong = guardianWarningOverlay?.querySelector("strong");
    const span = guardianWarningOverlay?.querySelector("span");
    if (strong) strong.textContent = "時間内に間に合った！";
    if (span) span.textContent = "核の防壁を破壊した。";
    guardianWarningOverlay?.classList.remove("hidden");
    guardianWarningOverlay?.classList.add("active", "victory");
  }, 1850);

  queueGuardianSequence(() => {
    guardianWarningOverlay?.classList.remove("active");
  }, 4550);

  queueGuardianSequence(() => {
    guardianWarningOverlay?.classList.add("hidden");
    showGuardianClearScreen(guardianId);
  }, 5050);
}

function triggerGuardianFailure() {
  if (!guardianBattle?.active) return;
  guardianBattle.active = false;
  guardianBattle.failing = true;
  stopGuardianClock();
  stopGuardianBallAndEffects();
  fadeOutStageBgm(0.45);
  guardianBombSource = playSe("bomb");

  const screen = document.getElementById("playScreen");
  screen?.classList.add("guardian-failing");
  guardianWarningOverlay?.classList.add("hidden");
  guardianWarningOverlay?.classList.remove("active", "victory");
  guardianPhaseBadge?.classList.add("hidden");
  guardianFailureOverlay?.classList.remove("hidden");
  guardianFailureOverlay?.classList.add("active");

  queueGuardianSequence(() => {
    guardianFailureOverlay?.classList.add("black");
    stopGuardianBomb();
  }, 500);
  queueGuardianSequence(() => {
    stopPlayMode();
...
function startGuardianBattle(guardianId) {
  const phases = guardianLevels[guardianId] || [];
  if (!areGuardianPhasesReady(guardianId)) {
    showStageToggleNotice(`GUARDIAN ${GUARDIAN_DEFINITIONS[guardianId].roman}の5フェーズファイルが揃っていません`, "warning");
    return;
  }

  resetGuardianPresentation();
  stopGuardianBallAndEffects();
  const definition = GUARDIAN_DEFINITIONS[guardianId];
  guardianBattle = { id: guardianId, phaseIndex: 0, active: true, transitioning: true, startedAt: 0 };
  currentLevel = JSON.parse(JSON.stringify(phases[0]));
  originalLevelData = JSON.parse(JSON.stringify(currentLevel.data));
  isEditorMode = false;
  isRealPlay = true;
  isOfficialPlay = true;
  ...
  const screen = document.getElementById("playScreen");
  screen?.classList.add("guardian-mode", "guardian-intro", "sanctum-mode");
  buildGuardianCore(guardianId);
  guardianCoreVisual?.classList.remove("hidden");
  guardianCoreVisual?.classList.add("assembling");
  guardianWarningOverlay?.classList.add("hidden");
  guardianPhaseBadge?.classList.add("hidden");
  if (guardianWarningCode) guardianWarningCode.textContent = `GUARDIAN ${definition.roman} / CORE BREACH PROTOCOL`;
  const warningStrong = guardianWarningOverlay?.querySelector("strong");
  const warningSpan = guardianWarningOverlay?.querySelector("span");
  if (warningStrong) warningStrong.textContent = "制限時間内に核の防壁を破壊せよ。";
  if (warningSpan) warningSpan.textContent = "間に合わなければ、核が爆発する！";

  setGlobalGlitchTarget(0.08);
  setStageTheme("boss_world");
  setShowDlcPillars(true, "boss");
  fadeOutStageBgm(0.65);
  showScreen("play");
  showPenWorkspace("play");

  gsap.killTweensOf([playIntro, playIntroTitle, playIntroSub, playIntroAuthor]);
  playIntroTitle.textContent = "01:45";
  playIntroSub.textContent = "";
  playIntroAuthor.textContent = "";
  gsap.set(playIntro, { top: "50%", yPercent: -50, opacity: 0 });
  gsap.set(playIntroTitle, { y: 0, opacity: 1, scale: 1.42 });

  queueGuardianSequence(() => {
    if (!guardianBattle?.active) return;
    guardianWarningOverlay?.classList.remove("hidden");
    guardianWarningOverlay?.classList.add("active");
    playStageBgm("boss");
    startGuardianClock();
    gsap.to(playIntro, { opacity: 1, duration: 0.35, ease: "power2.out" });
    gsap.to(playIntro, { top: window.innerHeight < 600 ? "112px" : "80px", yPercent: 0, duration: 1.05, delay: 2.55, ease: "power3.inOut" });
    gsap.to(playIntroTitle, { scale: 1, duration: 1.05, delay: 2.55, ease: "power3.inOut" });
  }, 850);

  queueGuardianSequence(() => {
    if (!guardianBattle?.active) return;
    guardianWarningOverlay?.classList.remove("active");
    queueGuardianSequence(() => guardianWarningOverlay?.classList.add("hidden"), 500);
    screen?.classList.remove("guardian-intro");
    showGuardianPhase(0, false);
  }, 5050);
}

function startGuardianBattleWithLoading(guardianId) {
  showLoading(async () => {
    const isReady = await ensureGuardianPhasesLoaded(guardianId);
    if (!isReady) {
      showStageToggleNotice(`GUARDIAN ${GUARDIAN_DEFINITIONS[guardianId].roman}の5フェーズファイルが揃っていません`, "warning");
      return;
    }
    startGuardianBattle(guardianId);
  }, 950);
}
...
function handleGridTilePointerDown(container, event, idx) {
  if (container?.classList?.contains("sanctum-materializing")) return;
  ...
  if (!isBallMoving && cellData.type === TYPE_START) {
    ...
    if ([TYPE_ROTATING_ARROW_CW_VAR, TYPE_ROTATING_ARROW_CCW_VAR].includes(cellData.type)) {
      cellData._manualRot = cellData.rot;
    }
...
const sanctumRevealTimers = new WeakMap();
function cancelSanctumReveal(container) {
  if (!container) return;
  clearTimeout(sanctumRevealTimers.get(container));
  sanctumRevealTimers.delete(container);
  container.classList.remove("sanctum-materializing");
  container.querySelectorAll('.sanctum-fragments, .sanctum-reveal-wave').forEach(el => el.remove());
  container.querySelectorAll('.sanctum-reveal-cell').forEach(el => {
    el.classList.remove('sanctum-reveal-cell');
    el.style.removeProperty('--sanctum-delay');
  });
}

function revealSanctumTiles(container) {
  cancelSanctumReveal(container);
  if (!currentLevel?._isDlc || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const size = currentLevel.size, center = (size - 1) / 2;
  let lastDelay = 0;
  currentLevel.data.forEach((cell, idx) => {
    if (cell.type === TYPE_VOID) return;
    const hitbox = container.children[idx];
    if (!hitbox) return;
    const tileX = idx % size - center;
    const tileY = Math.floor(idx / size) - center;
    const delay = Math.hypot(tileX, tileY) * 70;
    lastDelay = Math.max(lastDelay, delay);
    hitbox.style.setProperty('--sanctum-delay', `${delay}ms`);
    hitbox.classList.add('sanctum-reveal-cell');
    const fragments = document.createElement('div');
    fragments.className = 'sanctum-fragments';
    fragments.setAttribute('aria-hidden', 'true');
    const piece = document.createElement('i');
    piece.style.cssText = `--piece-dx:${-tileX * 8}px;--piece-dy:${-tileY * 8}px`;
    fragments.appendChild(piece);
    hitbox.appendChild(fragments);
  });
  const wave = document.createElement('div');
  wave.className = 'sanctum-reveal-wave';
  wave.setAttribute('aria-hidden', 'true');
  container.appendChild(wave);
  container.classList.add('sanctum-materializing');
  sanctumRevealTimers.set(container, setTimeout(() => cancelSanctumReveal(container), lastDelay + 760));
}
...
function startRealPlay(levelId, isOfficial = false, isFanmade = false) {
  ...
  showLoading(() => {
    const isMiniBoss = isOfficial && isBossLevel(currentLevel);
    ...
    showPenWorkspace("play");
    requestAnimationFrame(fitVisibleGrids);
    if (currentLevel._isDlc) revealSanctumTiles(playGrid);
...
function updatePlayIntroProximity(event) {
  ...
  if (document.getElementById("playScreen")?.classList.contains("guardian-mode")) {
    playIntro.classList.remove("pointer-near");
    return;
  }
...
function rememberManualRotations(data) {
  data?.forEach(cell => {
    if ([TYPE_ROTATING_ARROW_CW_VAR, TYPE_ROTATING_ARROW_CCW_VAR].includes(cell.type)) {
      cell._manualRot = cell.rot;
    }
  });
}
...
function applyKeptTileStates(freshData, oldData) {
  ...
  freshData.forEach((cell, i) => {
    const previous = oldData[i];
    if ([TYPE_ROTATING_ARROW_CW_VAR, TYPE_ROTATING_ARROW_CCW_VAR].includes(cell.type)
      && previous?.type === cell.type) {
      cell.rot = previous._manualRot ?? previous.rot;
      cell._manualRot = cell.rot;
    }
  });
...
function spawnBall(idx, container) {
  ...
  if (!isReplayMode) rememberManualRotations(currentLevel.data);
  if (activeJumpTween) {
    activeJumpTween.kill();
    activeJumpTween = null;
  }
...
function finishLevel(gx, gy, container) {
  if (guardianBattle?.active && currentLevel?._isGuardian) {
    completeGuardianPhase(gx, gy, container);
    return;
  }
...
function showClearScreen() {
  ...
  if (actionsContainer) {
    ...
    const guardianCandidateId = currentLevel?._isDlc && currentLevel._dlcNum === GUARDIAN_DEFINITIONS[1].afterStage
      ? 1
      : 0;
    const guardianTriggerId = guardianCandidateId && !isGuardianCleared(guardianCandidateId)
      ? guardianCandidateId
      : 0;
    ...
    btnNext.onclick = () => {
      if (guardianTriggerId) {
        startGuardianBattleWithLoading(guardianTriggerId);
        return;
      }
      if (isOfficialPlay && currentLevel && currentLevel._officialIndex !== undefined) {
        const nextIdx = currentLevel._officialIndex + 1;
