...
/* editor.js 内の fitGridToViewport 関数を修正 */
...
function fitGridToViewport(grid, extraTop = 0) {
  ...
  // ★修正: スマホ縦画面のマージンを 4px -> 30px に増やして端切れを防ぐ
  const marginW = isPortrait ? 30 : 32;
  ...
  // ★修正: スマホ縦なら拡大率を 0.85 -> 0.75 に下げて余裕を持たせる
  if (isPortrait) {
    scale *= 0.75;
  }
...
function startRealPlay(levelId, isOfficial = false) {
  ...
  showLoading(() => {
    ...
    // ★変更: 「by 作者名」はCSSの::beforeで "by " をつけるので、ここでは名前だけセット
