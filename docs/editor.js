/* editor.js */
import { showScreen, showLoading, playChin, fadeBgmToEx, fadeBgmToNormal, setSpaceBackground } from './main.js';
...
function fitVisibleGrids(){
  ...
  if (playActive) fitGridToViewport(playGrid, 80);
  if (editorActive) fitGridToViewport(editorGrid, 140);
}
...
if (window.visualViewport) {
  ...
  window.visualViewport.addEventListener("scroll", () => requestAnimationFrame(fitVisibleGrids));
}
...
function startRealPlay(levelId, isOfficial = false) {
  ...
  showLoading(() => {
    ...
    const isCompactUI = (window.matchMedia && window.matchMedia("(max-width: 740px)").matches) || window.innerHeight < 520;
    const introTop = isCompactUI ? "120px" : "60px";
    ...
    timeline.to([playIntro.children], { y: 0, opacity: 1, duration: 0.8, stagger: 0.2, ease: "power2.out" })
      .to(playIntro, { top: introTop, transform: "none", duration: 1.0, ease: "power2.inOut", delay: 1.0 })
