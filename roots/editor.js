/* editor.js */
import { showScreen, showLoading, playChin } from './main.js';
...
const TYPE_START = 2;
const TYPE_GOAL = 3;
const TYPE_SWITCH_ARROW = 4; // 可変矢印 (白)
const TYPE_FIXED_ARROW = 5;
const TYPE_TURN_VAR = 6;     // 転換 (白)
...
async function loadOfficialLevels() {
  if (officialLevelsLoaded) return;
  officialLevels = [];
...
function renderOfficialList() {
  ...
  if (officialLevels.length === 0) {
    officialListContainer.innerHTML = `<div style="padding:20px; text-align:center;">ステージが見つかりませんでした。</div>`;
    return;
  }

  const clearedStages = getClearedStages();
  // 最も進んでいる（クリアした番号の最大値）を取得してアンロック判定に使う
  const maxCleared = clearedStages.length > 0 ? Math.max(...clearedStages) : -1;
  ...
  officialLevels.forEach((level, idx) => {
    ...
    if (level._isEx) {
      item.classList.add("ex");
    }

    // クリア判定: 配列に含まれているか
    const isCleared = clearedStages.includes(idx);
    
    // ロック解除: 到達最大値+1までは開放 (EXは別フラグ)
    ...
    // クリア直後のフリップ演出
    if (isOfficialPlay && idx === lastClearedIndex) {
      item.classList.add("flip-reveal");
    }
    ...
    item.innerHTML = `
      <h4>${stageLabel}</h4>
      <p>${level.name}</p>
      <span class="sub-title">${level.sub || ""}</span>
    `;

    // 初回プレイ時の誘導
    if (idx === 0 && clearedStages.length === 0) {
      const guide = document.createElement("div");
      guide.className = "ftue-guide-bubble";
      guide.textContent = "ここからスタート！";
  ...
  if (lastClearedIndex !== -1) {
    setTimeout(() => { lastClearedIndex = -1; }, 1000);
  }
}
...
function loadLevelEditor(id) {
  ...
  showLoading(() => {
    editorTitle.textContent = currentLevel.name; renderGrid(editorGrid); showScreen("editorMain"); startEditorTimer();
  ...
  currentLevel.data.forEach((cellData, idx) => {
    ...
    if (isEditing) {
      ...
      tile.addEventListener("click", () => {
        ...
        else if (!isBallMoving && (cellData.type === TYPE_SWITCH_ARROW || cellData.type === TYPE_TURN_VAR)) {
...
function handleTileInteraction(idx) {
  ...
  if (cell.type === currentTileType) {
    if ([TYPE_START, TYPE_GOAL, TYPE_SWITCH_ARROW, TYPE_FIXED_ARROW, TYPE_TURN_VAR, TYPE_TURN_FIX, TYPE_U_TURN].includes(currentTileType)) {
...
function startRealPlay(levelId, isOfficial = false) {
  ...
  showLoading(() => {
    ...
    document.getElementById("playIntroAuthor").textContent = currentLevel.author || "";
    ...
    if (btnRetry) {
      btnRetry.onclick = () => { playChin(); retryRealPlay(); };
    }
    ...
    gsap.killTweensOf([playIntro, playIntro.children, ".play-timer-area"]);
    gsap.set(playIntro.children, { y: 30, opacity: 0 });
    gsap.set(playIntro, { top: "50%", transform: "translateY(-50%)" });
    gsap.set(document.querySelector(".play-timer-area"), { opacity: 0 });

    const timeline = gsap.timeline();
    timeline.to([playIntro.children], { y: 0, opacity: 1, duration: 0.8, stagger: 0.2, ease: "power2.out" })
      .to(playIntro, { top: "60px", transform: "none", duration: 1.0, ease: "power2.inOut", delay: 1.0 })
      .to(document.querySelector(".play-timer-area"), { opacity: 1, duration: 0.5 }, "-=0.5");
...
function startPlayTimer() {
  ...
  playTimerId = setInterval(() => {
    const diff = Math.floor((Date.now() - playStartTime) / 1000);
...
function onMoveComplete(nextX, nextY, nextIdx, nextCell, direction, container) {
  ...
  if (nextCell.type === TYPE_SWITCH_ARROW || nextCell.type === TYPE_FIXED_ARROW) {
...
function fallBall(direction, speedScale, container) {
  if (!ballEl) return;
  const dx = [0, 1, 0, -1]; const dy = [-1, 0, 1, 0];
  const speed = 0 * (speedScale || 1.5);
  const tx = (direction !== undefined) ? dx[direction] * speed : 0;
  const ty = (direction !== undefined) ? dy[direction] * speed : 0;
...
function finishLevel(gx, gy, container) {
  ...
  if (ballEl) {
    gsap.to(ballEl, { scale: 0, opacity: 0, duration: 0.5 });
  }

  // 公式プレイならクリア状況保存
  if (isOfficialPlay && currentLevel._officialIndex !== undefined) {
    // 個別にクリア状況を保存
    saveStageCleared(currentLevel._officialIndex);
    // 最後にクリアしたステージを記録（フリップ演出用）
